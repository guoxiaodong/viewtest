...
<template>
    <Modal v-model="addModal" title="编辑供应商" :styles="{width: '90%'}" @on-cancel="cancel('formInline')">
        <Form ref="formInline" :model="formInline" :rules="ruleInline" :label-width="130">
            ...
            <Row v-if="showContact" v-for="(item, index) in formInline.contact" :key="index">
                <Col span="9">
                    <FormItem
                        :label="'联系人 ' + (index + 1) + '：'"
                ...
                <Col span="9">
                    <FormItem
                        ...
                        :rules="{required: true, type: 'number', pattern: /^1[0-9]{10}$/, message: '请输入正确的手机号码', trigger: 'blur'}">
            ...
            <FormItem label="LOGO：" prop="logo">
                ...
                <div class="upload-default" v-if="showImg"><img :src="this.logo" width="130" heitgh="130"/></div>
            </FormItem>
    ...
    export default {
        ...
        methods: {
            open (EditData) {
                ...
                this.formInline.no = EditData.no
                this.formInline.name = EditData.name
                this.backname = this.formInline.name
                this.formInline.address = EditData.address.split(',')
                this.formInline.addressdetail = EditData.addressdetail
                this.formInline.email = EditData.email
                for (let v of JSON.parse(EditData.contact)) {
                    this.formInline.contact.push({name: v.name, tel: v.tel})
                }
                this.formInline.cooperatestatus_id = EditData.cooperatestatus_id
                this.formInline.contractstatus_id = EditData.contractstatus_id
                this.logo = EditData.logo
                this.formInline.logo = this.logo
                this.formInline.intro = EditData.intro
            },
            // 联系人
            handleAdd () {
                this.index++
            ...
            handleSubmit (name) {
                this.$refs[name].validate((valid) => {
                    if (valid) {
                        ...
                        SupplierManageHttp.putSupplier({supplier_id: v.id, no: v.no, name: v.name, address: v.address.toString(), addressdetail: v.addressdetail, email: v.email, contact: JSON.stringify(v.contact), cooperatestatus_id: v.cooperatestatus_id, contractstatus_id: v.contractstatus_id, logo: v.logo, intro: v.intro}).then((res) => {
                            this.confirmDis = false
                            this.addModal = false
                            this.$Message.success('提交成功!')
                            // this.$parent.tableTitle()
                            this.showContact = false
                            this.$parent.getSupplierInfo()
                            this.showImg = false
