...
<template>
    <Modal v-model="addModal" title="编辑分支" :styles="{width: '90%'}" @on-cancel="cancel('formInline')">
    ...
    export default {
        ...
        methods: {
            open (EditData) {
                ...
                this.formInline.id = EditData.id
                ...
                this.sites_name = EditData.search
            ...
            getSiteChecked () {
                let ids = []
                for (let v of this.EditData.places) {
                    ids.push(v.id)
                }
                for (let id of ids) {
            ...
            handleSubmit (name) {
                ...
                if (document.getElementById('citySel').value == '') {
                    ...
                    if (this.ztreeObj.getCheckedNodes(true).length != 0) {
                        ...
                        for (let v of this.EditData.places) {
                            this.formInline.place_ids.push(v.id)
                        }
                    }
                }
                if (this.fileurl) {
                ...
                this.$refs[name].validate((valid) => {
                    if (valid) {
                        ...
                        SupplierManageHttp.putBranch({id: v.id, supplier_id: this.$route.params.id, no: v.no, name: v.name, address: v.address.toString(), addressdetail: v.addressdetail, contact: JSON.stringify(v.contact), place_ids: v.place_ids, logo: v.logo, intro: v.intro}).then((res) => {
                            this.confirmDis = false
                            this.addModal = false
                            this.$Message.success('提交成功!')
                            this.$parent.getBranchInfo()
                            this.showImg = false
                            this.cancel(name)
                        }).catch((error) => {
                            this.confirmDis = false
