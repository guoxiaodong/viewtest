...
<template>
    <Modal class="formModal" v-model="addModal" title="添加服务信息" :styles="{width: '90%'}" @on-cancel="cancel('formDynamic')">
        <Form ref="formInline" :model="formInline" :rules="ruleInline" :label-width="120">
            <FormItem prop="service_ids" label="供应商服务：">
                <!-- <Select v-model="formInline.service_ids" @on-change="changeOption">
                    <Option v-for="item in this.SupplierSerList" :value="item.id" :key="item.id">{{ item.title }}</Option>
                </Select> -->
                <Select v-model="formInline.service_ids" @on-change="changeOption">
            ...
            handleSubmit (formInline) {
                ...
                this.$refs[formInline].validate((valid) => {
                    if (valid) {
                        ...
                        SupplierManageHttp.addBranchSer({supplierbranch_id: parseInt(this.$route.params.sid), service_ids: v.service_ids, servicelevel: v.servicelevel.toString(), servicestatus: parseInt(v.servicestatus)}).then((res) => {
