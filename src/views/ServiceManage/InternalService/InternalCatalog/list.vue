<style>
    ...
    .internalCatalog .inneritem {
        background-position-x: 41.72%;
    }
    .internalCatalog .ivu-table-wrapper{
        border-top: 0px;
    }
    .internalCatalog .ivu-table-wrapper.ivu-table-with-header{
        border-left: 1px solid #dddee1!important;
        left: 0px!important;
        width: 99.956%!important;
    }
    .internalCatalog .ivu-table-wrapper.ivu-table-with-header .ivu-table{
        max-width: 99.8%!important;
    }
    .internalCatalog .internalCatalogSearch .ivu-icon.ivu-icon-search.ivu-input-icon.ivu-input-icon-normal{
        top:0px!important;
    }
    .internalCatalog .search.ivu-input-wrapper.ivu-input-type{
        width: 89%;
    }
    .internalCatalog .edit{
        float:right;
        margin: 10px 25px;
    }
    .internalCatalog .internalCatalog-title
    {
        margin-left: 25px;
    }
    .internalCatalog .internalCatalog-title p{
        margin:5px 0;
    }
    .internalCatalog .internalCatalog-name{
        margin-left: 40px;
        margin-top: 10px;
        margin-bottom: 10px;
    }
    .internalCatalog .internalCatalog-intro{
        margin-left: 25px;
    }
...
<template>
    <div class="internalCatalog">
        ...
        <div class="innercontent materialBasic" >
            <Row class="inneritem" >
                <Col span="10">
                <Input class="internalCatalogSearch" v-model="searchTreeNode" placeholder="请输入搜索内容" icon="search" style="width: 98%;margin:4px;" @on-change="searchNode()"></Input>
                <div id='internalCata' class='ztree' style="overflow: auto;" ></div>
                </Col>
                <Col  span="14" style="padding: 10px">
                <div class="internalCatalog-title">
                    <div><span v-if="editstatus == true" class="detail-title">{{detail.name}}</span>
                        <Input v-if="editstatus == false" :maxlength="40" v-model="editName" placeholder="请输入" style="width: 300px"></Input>
                ...
                <hr/>
                <div class="internalCatalog-name">
                    <p>服务分类编号： {{detail.no}}</p>
                    <p v-if="detail.no">服务分类名称： {{detail.className}}</p>
                    <p v-if="detail.type == 3">服务分类名称： {{detail.className}}</p>
                </div>
                <p v-if="editstatus == true" class="internalCatalog-intro">{{detail.intro}}</p>
                <p v-if="editstatus == false" class="internalCatalog-intro">
        ...
        <Modal
            ...
            <div class="addtreeNode">
                <Form ref="formValidate" :model="formValidate" :rules="ruleValidate" label-position="left">
                    ...
                    <p v-if="addtype == 1">所属服务组分类：<span>{{addNodeClass}}</span></p>
                    <FormItem label="分类名称：" prop="addClassName" class="needRequired" style='margin-bottom: 10px'>
                    ...
                    <FormItem label="简介：" prop="addIntro">
                        <Input v-model="formValidate.addIntro" :maxlength="800" type="textarea"  placeholder="请输入"></Input>
                    </FormItem>
                </Form>
            </div>
            <div slot="footer">
...
<script>
    ...
    export default {
        ...
        created () {
            this.$emit('updateActive', '7-4')
        },
        data () {
            const validatePass = (rule, value, callback) => {
                if (value === '') {
                    if (this.addtype == 0) {
                        callback(new Error('服务分类名为必填项'))
                    }
                } else {
                    if (this.addtype == 0) {
                        var data = {
        ...
        methods: {
            ...
            add (node) {
                if (node.level == 0) {
                    this.addnumber = 'EST' + new Date().getTime() + Math.floor(Math.random() * 10) + Math.floor(Math.random() * 10) + Math.floor(Math.random() * 10) + Math.floor(Math.random() * 10)
                    this.addTitle = '新增服务分类'
                    this.addtype = 0
                }
                ...
                this.$refs[name].validate((valid) => {
                    if (valid) {
                        if (this.addtype == 0) {
                            ...
                            ServiceHttp.addInternalClass(data).then((data) => {
            ...
            zTreeBeforeDrop (treeId, treeNodes, targetNode, moveType) {
                if (treeNodes[0].level != 2 || targetNode.level != 1) {
                    return false
                }
                var data = {
            ...
            treeInt (name = null) {
                ServiceHttp.internalCata().then((data) => {
                    this.ztreeObj = $.fn.zTree.init($('#internalCata'), this.setting, data)

                    this.ztreeObj.expandNode(this.ztreeObj.getNodeByParam('tId', 'internalCata_1'))

                    // 默认的打开详情
                    // 默认的打开详情
                    if (!name) {
                        let node = this.ztreeObj.getNodeByParam('tId', 'internalCata_2')
                        $('a#' + node.tId + '_a').click()
                    } else {
                ...
                document.getElementById('internalCata').style.height = (document.body.clientHeight - 213) + 'px'
            },
            onResize () {
                document.getElementById('internalCata').style.height = (document.body.clientHeight - 213) + 'px'
            },
