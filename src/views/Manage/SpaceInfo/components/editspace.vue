<template>
    <Modal v-model="addobject" title="编辑空间" @on-cancel="cancel('formInline')">
            ...
            handleSubmit (name) {
                ...
                this.$refs[name].validate((valid) => {
                    if (valid && this.submitCount == 1) {
                        this.confirmDis = true
                        SpaceInfoHttp.putPlace({name: this.formInline.space_name, shortname: this.formInline.space_shortname, type_id: this.formInline.space_typeid, id: this.id, fid: this.fid, level: this.level}).then((res) => {
