<style>
    ...
    .map-search {position: absolute; left: 0; top: 165px; z-index: 99; width: 320px; max-height: 100px; margin-left: 170px;}
</style>
<template>
    <Modal v-model="addobject" title="新增楼宇" @on-cancel="cancel('formInline')" :styles="{width: '90%'}">
        <Form class="formBuild" ref="formInline" :model="formInline" :rules="ruleInline" :label-width="120">
            ...
            <FormItem prop="diviation" label="有效半径：">
                <Input type="text" number v-model="formInline.diviation" placeholder="请输入" style="width:290px; color: #495060; font-size: 14px;"></Input><span style="float: right;">米</span>
            </FormItem>
        ...
        <baidu-map v-if="mapShow" @ready="mapHandler" @click="mapClick" :scroll-wheel-zoom="true" :center="map.center" :zoom="map.zoom" ak="F1f9d239ddb4b86bd403120058d1ee08">
            ...
            <bm-local-search class="map-search" @infohtmlset='hideMap' id="mapSearch" :pageCapacity="5" :location="location" :keyword="formInline.building_name" :auto-viewport="true"></bm-local-search>
        </baidu-map>
    ...
    export default {
        ...
        data () {
            ...
            return {
                ...
                ruleInline: {
                    ...
                    longitude: [
                        { required: true, type: 'number', message: '请点击右侧的楼宇名称来获取相应的经度!', trigger: 'blur' }
                    ],
                    latitude: [
                        { required: true, type: 'number', message: '请点击右侧的楼宇名称来获取相应的纬度!', trigger: 'blur' }
                    ],
                    diviation: [
        ...
        methods: {
            open (node) {
                ...
                this.location = node.getParentNode().name
                this.addobject = true
                this.submitCount = 0
                this.mapShow = true
            },
            ...
            handleSubmit (name) {
                ...
                this.$refs[name].validate((valid) => {
                    if (valid && this.submitCount == 1) {
                        this.confirmDis = true
                        SpaceInfoHttp.addPlace({name: this.formInline.building_name, shortname: this.formInline.building_shortname, longitude: this.formInline.longitude, latitude: this.formInline.latitude, diviation: this.formInline.diviation, fid: this.id, level: this.level}).then((res) => {
                            this.addobject = false
                            this.$refs[name].resetFields()
                            this.$Message.success('提交成功!')
                            this.confirmDis = false
                            this.$parent.addBuildNode(res)
                            this.mapShow = false
                        }).catch((error) => {
                            this.confirmDis = false
                            this.submitCount = 0
