<template>
    <Modal v-model="addobject" title="新增空间" @on-cancel="cancel('formInline')">
            ...
            handleSubmit (name) {
                ...
                this.$refs[name].validate((valid) => {
                    if (valid && this.submitCount == 1) {
                        this.confirmDis = true
                        SpaceInfoHttp.addPlace({name: this.formInline.space_name, shortname: this.formInline.space_shortname, type_id: this.formInline.space_typeid, fid: this.id, level: this.level}).then((res) => {
