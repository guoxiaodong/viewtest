...
<template>
    <Modal class="formModal"  v-model="addobject" :styles="{width: '90%'}" title="新增对象" @on-cancel="cancel('formDynamic')">
        <Form ref="formDynamic" :model="formDynamic" :label-width="110"  :rules="ruleInline">
            <FormItem prop="objectname" label="名称：">
                <Input type="text" style="width: 100%;" v-model="formDynamic.objectname" placeholder="请输入名称"></Input>
                <!-- :rules="{type: 'string', required: true, message: '请填写对象名称'}" -->
            </FormItem>
            <ul>
                    ...
                    <Row>
                        <Col span="24" offset="1">
                            <FormItem v-for="(v, k) in formDynamic.items[index].standards"
                                ...
                                <Row>
                                <Col span="17">
                                    <Select v-model="formDynamic.items[index].standards[k].value" :ref="'standard_' + index + '_' + k" not-found-text="" filterable remote :remote-method="(v) => {searchStandard(v, index)}">
            ...
            handleSubmit (name) {
                ...
                this.$refs[name].validate((valid) => {
                    if (valid && this.submitCount == 1) {
                        ManageObjectHttp.addObject(this.formDynamic).then((data) => {
