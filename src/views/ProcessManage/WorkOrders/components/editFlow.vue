    ...
    <Modal
        ...
        <div style="margin-left: 40px; margin-right: 40px;">
            <Tabs value="name1" :animated="false">
                    ...
                    <div class="">
                        <Form ref="formInline" :model="formInline" :rules="ruleInline" :label-width="100" class="createForm">
                            ...
                            <FormItem  v-for="(item, index) in formData" :label="item.name" :key="item.index">
                                <!--输入框-->
                                <Input v-if="item.filed_type == 'in1'" v-model="formInline.field" placeholder="请输入"></Input>
                                <!--单选下拉框-->
                                <Select v-model="formInline.field" filterable v-else-if="item.filed_type == '21'">
                                   <Option v-for="item1 in dataList" :value="item1.name" :key="item1.id">{{ item1.name }}</Option>
                                </Select>
                                <!--多选下拉框-->
                                <Select v-model="formInline[item.field]" @on-change="vc(index)" multiple filterable v-else-if="item.filed_type == '2'">
                ...
                <TabPane label="环节" name="name2">
                    <div class="PMtable">

                    </div>
                </TabPane>
                <TabPane label="流程预览" name="name3">标签三的内容</TabPane>
            </Tabs>
        </div>
    </Modal>
</template>
